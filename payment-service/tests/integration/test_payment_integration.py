...
def _discover_payment_paths(openapi: dict):
...
def _find_payment_model() -> Optional[Type[Any]]:
    # Пробуем распространённые имена
    for attr in ("Payment", "Transaction", "Charge", "Invoice"):
    ...
    try:
        for m in Base.registry.mappers:  # type: ignore[attr-defined]
            ...
            if any(s in name for s in ("payment", "transaction", "charge", "invoice")) or any(
                s in str(tbn).lower() for s in ("payment", "transaction", "charge", "invoice")
            ):
                return cls
    except Exception:
        pass
...
def test_create_persists_and_refund(monkeypatch):
    ...
    _, create_path, get_by_id_path, refund_path = _discover_payment_paths(spec)
    if not create_path:
        pytest.skip("В OpenAPI нет POST-эндпоинта для создания платежа/чарджа.")

    created = None
    # Если create-путь с path params — подставим order_id = 5
    if "{" in create_path and "}" in create_path:
        ...
        r = client.post(path, json={"amount": 12.5, "currency": "USD"})
    ...
    if not created:
        for body in [
            {"order_id": 5, "amount": 12.5, "currency": "USD", "source": "tok_visa"},
            {"order_id": 5, "amount": 1250, "currency": "usd", "payment_method": "card"},
            {"orderId": 5, "amount": 12.5, "currency": "USD"},
        ]:
    ...
    assert created, f"Создание платежа через {create_path} не удалось."
    ...
    if isinstance(created, dict):
        ...
        for k in ("payment_id", "id", "transaction_id", "uuid"):
            if k in container:
                pid = container[k]
                break
    assert pid is not None, "Ответ API не содержит id созданного платежа."

    # Проверяем, что запись действительно в БД
    Model = _find_payment_model()
    if Model is None:
        pytest.skip("Не удалось найти ORM-модель платежа (Payment/Transaction/Charge).")

    s = _get_session()
    try:
        row = s.get(Model, pid) if hasattr(s, "get") else s.query(Model).get(pid)  # type: ignore[attr-defined]
        assert row is not None, "Запись платежа не найдена в БД."
    finally:
        s.close()
    ...
    # REFUND — если такой путь есть, делаем возврат
    if refund_path:
        url = (
            refund_path
            .replace("{id}", str(pid))
        ...
        r = client.post(url, json={"reason": "requested_by_customer"})
